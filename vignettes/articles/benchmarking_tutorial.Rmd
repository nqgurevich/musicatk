...
Results from all analyses related to the same ground truth signatures and exposures can be stored in one `full_benchmark` object. Benchmarking can be performed on multiple prediction instances and stored in one place, allowing for easy comparison between discovery methods or discovery parameters. 

The `full_benchmark` object is initialized using the ground truth signatures and exposures, both of which must be a matrix, as well as the mutation count table and modality. The signatures matrix should contain signatures as columns and mutation types as rows. Each cell details that mutation type's percent contribution to the given signature. The exposures matrix should contain signatures as columns and samples as rows. Each cell details the number of mutations in the sample that are attributed to the given signature. The count table should contain mutation types as rows and samples as columns.

The example below prepares a `full_benchmark` object using a synthetic breast cancer dataset provided in the package.
...
full_benchmark <- create_benchmark(true_signatures, true_exposures, count_table, "SBS96")

```

# Step 2. Prepare prediction to benchmark

In order to benchmark the results of signature discovery, the prediction results must be stored in a `musica` object. 

## If signature discovery has already been completed externally

If signature discovery has already been performed and therefore predicted signatures and predicted exposures already exist, this information must be stored in a `musica` object using the `create_musica_from_counts` (or `create_musica_from_variants`) function, followed by the `add_result` function. Example data is used here. Note that variant data or a mutation count table must be available in order to initialize the `musica` object.
...
# read in predicted exposures
data("example_predicted_exp")
predicted_exposures <- example_predicted_exp

# create musica object
musica <- create_musica_from_counts(count_table, "SBS96")

# add a result_model object for the prediction
add_result(predicted_sigs, predicted_exposures, musica, result_name = "result", modality = "SBS96", model_id = "external_prediction")

```

## If signature discovery has not yet been performed

Alternatively, if signature discovery has not yet been performed, it cane be done via the NMF or LDA algorithms within the musicatk package. Mutation data can be read in and stored in a `musica` object either from a file of variants or from a 
...
# prediction
discover_signatures(musica, "SBS96", num_signatures = 8, algorithm = "nmf", result_name = "result", model_id = "nmf8")
...
benchmark(full_benchmark, musica, prediction_model_id = "nmf8", prediction_modality = "SBS96", prediction_result_name = "result")

```

## Returning a new object from the benchmarking process

If a new `full_benchmark` object is desired, the `make_copy` argument can be used to return a new object, leaving the inputted one unchanged. Below, the nmf8 prediction will be benchmarked and the result is saved to a new object. For better readability, full plots are suppressed.

```{r perform benchmarking with copy}

new_benchmark <- benchmark(full_benchmark, musica, prediction_model_id = "nmf8", prediction_modality = "SBS96", prediction_result_name = "result", make_copy = TRUE, plot = FALSE)
...
nmf8_benchmark <- benchmark_get_entry(full_benchmark, "nmf8")
...
# extract the nmf8 prediction before benchmarking adjustments have been made
nmf8_initial_prediction <- benchmark_get_prediction(nmf8_benchmark, "initial")

# extract the nmf8 prediction after duplicate signatures have been corrected, but before composites have been corrected
nmf8_intermediate_prediction <- benchmark_get_prediction(nmf8_benchmark, "intermediate")

# extract the nmf8 prediction at the end of its benchmarking process
nmf8_final_prediction <- benchmark_get_prediction(nmf8_benchmark, "final")

```

## Comparison table between true and predicted signatures

To extract a comparison between the predicted and true signatures from any step in the benchmarking process, the `benchmark_get_comparison` function can be used.

``` {r compare results}

benchmark_get_comparison(full_benchmark, "nmf8", "Initial")
...
benchmark_plot_comparison(full_benchmark, "nmf8", "Initial")
...
benchmark_plot_signatures(full_benchmark, "nmf8", "Intermediate")
...
benchmark_plot_exposures(full_benchmark, "nmf8", "Initial")

```

To recreate the plots comparing predicted and true exposures before/after duplicate or composite signatures are corrected, the `benchmark_plot_duplicate_exposures` and `benchmark_plot_composite_exposures` functions can be used, respectively. There are no duplicate or composite signatures for this example, so no plots are produced here.

```{r plot duplicate/composite exposures before/after adjustment}

benchmark_plot_duplicate_exposures(full_benchmark, "nmf8")
benchmark_plot_composite_exposures(full_benchmark, "nmf8")
